    ...
    public partial class SinglePlayForm : Form
    {
        ...
        private bool judge() //승리 판정  함수
        {
            ...
            for (int i = 4; i < edgeCount; i++)
                for (int j = 0; j < edgeCount - 4; j++) //y=-x직선
                    if (board[i, j] == nowPlayer && board[i - 1, j + 1] == nowPlayer && board[i - 2, j + 2] == nowPlayer && board[i - 3, j + 3] == nowPlayer && board[i - 4, i + 4] == nowPlayer)
                        return true;
            return false;
        }
